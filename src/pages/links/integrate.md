...
- ### Redirections
    ...
    - Navigation URLs must be websites, not deep links
        ...
        | $fallback_url_xx | | Change the redirect endpoint for all platforms based on a lower-case Alpha-2 country code conforming to [ISO 3166](https://www.iso.org/obp/ui/#search). For example `$fallback_url_de="..."` for Germany. You should also set `$fallback_url` to act as the global redirect in addition to the country-specifc ones.
